...
TNetXNGFile::TNetXNGFile(const char *url,
                         ...
                         Bool_t      parallelopen,
                         const char* lurl) :
   TFile((lurl ? lurl: url), "NET", title, compress),lurl(lurl)
{
...
Bool_t TNetXNGFile::GetVectorReadLimits()
{
   ...
   if(0==lrl.GetProtocol().compare("file") && 0==lrl.GetHostId().compare("localhost")){
    if (gDebug>=1) Info("GetVectorReadLimits", "Local redirect, using default values");
     return kTRUE;
   }
